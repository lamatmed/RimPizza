...
export default function Navbar() {
  ...
  // Fermer la navbar en cliquant à l'extérieur ou sur un élément (sauf la recherche)
  useEffect(() => {
    ...
    if (isMobileMenuOpen) {
      // Utiliser setTimeout pour éviter une fermeture immédiate
      setTimeout(() => {
  ...
  const handleLogout = async () => {
    try {
      ...
      setIsMobileMenuOpen(false); // Fermer le menu même en cas d'échec
    }
  };
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    if (searchQuery.trim().length) {
      ...
      setIsMobileMenuOpen(false); // Fermer le menu mobile lors d'une recherche
    }
  };

  const handleMenuItemClick = () => {
    setIsMobileMenuOpen(false); // Fermer le menu mobile lors d'un clic sur un élément
  };
          ...
          <div className='hidden lg:flex items-center space-x-8'>
            ...
            <div className='relative group'>
              <form onSubmit={handleSearch} className='relative'>
                ...
                <Input
                  type='text'
                  placeholder='Rechercher des pizzas...'
                  value={searchQuery}
